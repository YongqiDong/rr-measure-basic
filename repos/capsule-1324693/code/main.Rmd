...
d <- readr::read_csv("../data/EDC_clean2.csv",na = c("-99","-88","-77","")) %>% as.data.frame()
...
myalpha <- function(vn, fd, omega = FALSE) {
  ...
  keys <- suppressWarnings(
    data.frame(keys = dv %>%
                 alpha(check.keys = TRUE) %$% keys,
               row.names = colnames(dv)))
  alphaObj <- scoreItems(keys = as.data.frame(keys), items = dv, missing = TRUE, impute = "none")
  keys <- keys$keys
  names(keys) <- colnames(dv)
  print(keys)
...
fd$SelfEsteem <- myalpha(vn = "P03_", fd = fd) #create variable called SelfEsteem in data set fd that is the mean of all P03 items
...
library(lm.beta) #to obtain standardized coefficients from an lm object
...
labs(x = 'Self Esteem', y = 'Standardized Scores on Partner Enhancement Index')+
  apatheme
p4
ggsave("../results/fig4.png", p4, width = 20, height = 12, units = "cm")
