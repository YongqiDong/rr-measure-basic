getSettings <- function(paper=1,exotic=FALSE,fake=FALSE,reduced=FALSE){
if (!reduced){
  scanner <- c(
    #Weather
    'minTemp_C',
  ...
  subj <- c(
    ...
    #These are nice physiological ones.
    'liquid_last24hs_liters',
    'weight_kg',
    'bloodPressure_systolic_mmHg',
    'bloodPressure_diastolic_mmHg',
    'estradiol', 		#(saliva)
    'testosterone',	#(saliva)
    'generalStress_last24hs',
    'SurfaceHoles',

    #caffein
    'caffein_last24hs_cups',
    'caffein_last2hs_cups',
    #todo choco
    'chocolate_cacao_24hr',
    'chocolate_cacao_2hr',
    'alcohol_last24hs_drinks',
    #Fitbit data
    'number_steps')

  predictors <- c(subj,scanner)
  onlyFemale <- c('estradiol','testosterone')
}else{
  scanner <- c()
  subj <- c('daysFromFirstScan','scanTime')
  predictors <- subj
  onlyFemale <- c()
}
...
if (paper==1){
  if (!exotic){
    outcomes <-     outcomes <- c( "GM","WM","Total","FS_TotalGrayVol","FS_CortexVol","ICV_ASHS")
  }
  else{
    outcomes <- c('FS_bilat_rostralmiddlefrontal_thickness','FS_bilat_rostralmiddlefrontal_thickness','VBM_HC_AAL','VBM_mOFC_AAL')
  }
}else{
  if (!exotic){
    outcomes <- c('FS_bilat_rostralmiddlefrontal_thickness','FS_bilat_medialorbitofrontal_thickness','FS_bilat_Hippocampus')
  }
  else{
    outcomes <- c('FS_bilat_rostralmiddlefrontal_thickness','FS_bilat_medialorbitofrontal_thickness','FS_bilat_Hippocampus',
                  # "FS_long_bilat_rostralmiddlefrontal_thickness","FS_long_bilat_Hippocampus","FS_long_bilat_medialorbitofrontal_thickness",
                  # 'longVBM_mOFC_AAL',,'longVBM_HC_AAL',longVBM_BA9_46_AAL'
                  'VBM_BA9_46_AAL','VBM_mOFC_AAL','VBM_HC_AAL')
  }
}
if(fake){
  outcomes <- c('Signal','No_Signal')
}
return(list(preds=predictors,outcomes=outcomes,scanner=scanner,subj=subj,onlyFemale=onlyFemale))
}

getReliability <- function(lmerModel){
  stopifnot(class(lmerModel)=='lmerMod')
  #get total variance
  totalVariance <- var(lmerModel@resp$y)
  #get error variance
  errorVariance <- summary(lmerModel)$sigma^2
  return(c(1-errorVariance/totalVariance,errorVariance,totalVariance))
}

getReliability2 <- function(lmerModel){
  stopifnot(class(lmerModel)=='lmerMod')
  trueVar <- VarCorr(lmerModel)$subject[1,1]
  errVar <- summary(lmerModel)$sigma^2
  return(c(trueVar/(trueVar+errVar),trueVar,errVar))
}

getReliability3 <- function(lmerModel){
  stopifnot(class(lmerModel)=='lmerMod')
  trueVar <- VarCorr(lmerModel)$subject[1,1]*6/5
  errVar <- summary(lmerModel)$sigma^2
  return(c(trueVar/(trueVar+errVar),trueVar,errVar))
}

transferFactors <- function(df){
  #transfer every ordered factor into numeric, because we are only interested in linear contrasts
  nfac <- 0
  for (i in 1:ncol(df)){
    if (is.ordered(df[,i])){
  ...
  print(paste(nfac,"Ordered Factors transfered to numeric i.e., they are treated as continous"))
  return(df)
}

getBrain <- function(myData){
  isbrain <- vapply(myData, function(x){attr(x,'brain')}, FUN.VALUE = TRUE)
  isbrain <- isbrain[isbrain]
  return(names(isbrain))
}

getMiss <- function(df,candidates=NULL){
  onlyFemale <- c("estradiol","testosterone")
  if (any(candidates %in% onlyFemale)){
    df[df$gender=='male',onlyFemale] <- 0 # male subjects do not have female stuff by default
  }
  cdf <- df
  df <- df[,c(candidates)]
  NAs <- is.na(df)
  naDF <- cdf[which(as.logical(rowSums(NAs))),c('subject','scanDate',candidates)]
  naCols<-colSums(NAs)
  missRel<-naCols/nrow(df)
  missRel <- round(missRel*100,2)
  allMiss <- data.frame(miss=naCols,missRel=missRel)
  allMiss <- allMiss[order(allMiss$miss,decreasing = TRUE),]
  return(list(allmiss=allMiss,details=naDF))
}


assertNoMis <- function(df,candidates){
  df <- df[,candidates]
...
outcomeTranslate <- function(outcome){
  switch(outcome,
         "FS_bilat_rostralmiddlefrontal_thickness"={
           myTitle<-'FreeSurfer  \n Middle Frontal Gyrus, Rostral Division'
         },
         "FS_bilat_medialorbitofrontal_thickness"={
           myTitle<-'FreeSurfer \n Medial Orbitol Frontal Cortex'
         },
         "FS_bilat_Hippocampus"={
           myTitle<-'FreeSurfer \n Hippocampus'
         },
         "VBM_HC_AAL"={
           myTitle<-'VBM \n Hippocampus'
         ...
         "VBM_mOFC_AAL"={
           myTitle<-'VBM \n Middle Frontal Gyrus, Orbial Part'
         },
         "FS_CortexVol"={
           myTitle<-'FS-Cortex'
         },
...
variableTranslate <- function(outcome){
  switch(outcome,
         "maxTemp_C"={
           myTitle<-'Max. Outside Temp.'
         },
         "minTemp_C"={
           myTitle<-'Min. Outside Temp.'
         },
         "scanTime"={
           myTitle<-'Time of Day'
         },
         'daysFromFirstScan'={
           myTitle<-"Days Since First Scan"
         },
         'liquid_last24hs_liters'={
           myTitle<-"Liquid Intake Last 24h"
         },
         'bloodPressure_systolic_mmHg'={
           myTitle<-"Blood Pressure Systolic"
         },
         'bloodPressure_diastolic_mmHg'={
           myTitle<-"Blood Pressure Diastolic"
         },
         'estradiol'={
           myTitle<-"Estradiol"
         },
         'testosterone'={
           myTitle<-"Testosterone"
         },
         'generalStress_last24hs'={
           myTitle<-"Stress Last 24h"
         },
         'caffein_last24hs_cups'={
           myTitle<-"Caffein Intake Last 24h"
         },
         'caffein_last2hs_cups'={
           myTitle<-"Caffein Intake Last 2h"
         },
         'chocolate_cacao_24hr'={
           myTitle<-"Cacao Intake Last 24h"
         },
         'chocolate_cacao_2hr'={
           myTitle<-"Cacao Intake Last 2h"
         },
         'alcohol_last24hs_drinks'={
           myTitle<-"Alcohol Intake last 24h"
         },
         'number_steps'={
           myTitle<-"Steps Previous Day"
         },
         'sunshine_hrs'={
           myTitle<-"Hours of Sunshine"
         },
        'MR_Room_Temperature'={
          myTitle<-"Room Temperature"
        },
        'MR_Room_Humidity'={
          myTitle<-"Room Humidity"
        },
        'MR_HeliumLevel'={
          myTitle<-"Helium Level"
        },
        'weight_kg'={
          myTitle<-"Weight"
        },
        'SurfaceHoles'={
          myTitle <- 'Surface Holes'
        },
         {
           myTitle <- outcome
         })
...
yAxisTranslate <- function(outcome){
  label <- 'Volume in $cm^{3}$'
  if (outcome %in% c("FS_TotalGrayVol","FS_CortexVol","ICV_ASHS")){
    thousands <- TRUE    
  }else{
    thousands <- FALSE    
  }
  
  return(list(label=label,thousands=thousands))
}
...
getVersions <- function(rfPath){
  result <- list()
  rFile <- basename(rfPath)
  folder <- dirname(rfPath)
  cFolder <- tail(strsplit(folder,'/')[[1]],n=1)
  while(!identical(cFolder,'data')){ #while cFolder not data
    ##get version of function
    cFile <- paste(folder,rFile,sep='/')
    load(cFile)
    strCall <- as.character(repres$call)
    stopifnot(identical(cFolder,strCall[1]))
    tRes <- list()
    result[[cFolder]] <-  cFile
    ##go one level down head(x,-100)
    splitFolder <- head(strsplit(folder,'/')[[1]],-1)
    folder <- paste(splitFolder,collapse="/")
    cFolder <- tail(splitFolder,n=1)
    if (grepl("=",strCall[2])){
      rFile <- substring(strsplit(strCall[2],'=')[[1]][2],2)
    }else{
      rFile<-strCall[2]  
    }
  } 
...
corrplot <- function(corr,
  ...
  win.asp = 1,addDot=FALSE,
  ...)
{
  stopifnot(!addDot | addCoefasPercent)
  ...
  if (method == "number" && plotCI == "n") {
    myLabels <- round((DAT - int) * ifelse(addCoefasPercent, 100, 1) / zoom,
                     number.digits)
    if (addDot){
      negativeLabels <- myLabels<0
      myLabels <- abs(myLabels)
      ...
      myLabels <- vapply(myLabels, addZeros, FUN.VALUE = '.00') #fill zeros
      myLabels <- paste0('.',myLabels) #add .
      myLabels[myLabels=='.100'] <- '1'
      myLabels[negativeLabels] <- paste0('-',myLabels[negativeLabels])
    }
