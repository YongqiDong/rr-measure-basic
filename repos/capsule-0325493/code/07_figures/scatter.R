...
#load data set
load("~/Downloads/day2day/data/joinDataSources/cleanData/3.RData")
theData <- repres$res
#run lm model
...
par(pch=16)
pdf(file.path(projDir,'/results/9_Scatter.pdf'),width=paperWidth, height=paperHeight*0.7,onefile=TRUE)
par(mfrow=c(2,2))
...
crPlot(theModelCortex,'scanTime',ylab='Partial Residual FS-Cortex',xlab=c('Time of Scan'),main='',pch=16,col='#BEBEBE',col.lines = c('black'),grid=FALSE,bty='l')
...
crPlot(theModelGray,'scanTime',ylab='Partial Residual FS-TotalGray',xlab=c('Time of Scan'),main='',pch=16,col='#BEBEBE',col.lines = c('black'),grid=FALSE,bty='l')
par(opar) 
dev.off()
