##################################################
## The Subconscious Effect of Subtle Media Bias ##
##         on Perceptions of Terrorism          ##  
##################################################
...
## 1) PROOF THAT LARGER CIs ARE A FUNCTION OF TRUNCATED SAMPLE SIZE:

## Fit full model:
full <- amce(Chosen2 ~ Wording + Label + Location + Severity + Type
             + Wording*Label,
             data = medord, cluster = T, respondent.id = "ResponseId",
             design = "uniform")

## Set seed for replication of sample() command:
set.seed(123)

## Fit restricted model with a random subset of 1/6 of the original N:
rest <- amce(Chosen2 ~ Wording + Label + Location + Severity + Type
             + Wording*Label,
             data = medord, cluster = T, respondent.id = "ResponseId",
             design = "uniform",
             subset = sample(c(TRUE,FALSE), nrow(medord),
                             replace = TRUE, prob = c(1/6, 5/6)))

## Extract standard errors of interaction in full sample:
se_full <- full$estimates$`Label:Wording`[c(2,4)]

## Extract standard errors of attributes in restricted sample:
se_rest <- c(rest$estimates$Label[c(2,4)],
             rest$estimates$Location[c(2,4)],
             rest$estimates$Severity[c(2,4)],
             rest$estimates$Type[c(2,4)],
             rest$estimates$Wording[1])

## Compute and compare means of the respective SEs:
mean(se_full)
mean(se_rest)


## 2) PROOF THAT DIFFERENCE BETWEEN LABELS IS STATISTICALLY SIGNIFICANT:

## Show significant difference between Terrorist and Islamist label on objectivity:
## Set "Terrorists" as baseline:
medord$Label2 <- relevel(medord$Label, "Terrorists")

## Re-fit the model:
model6terr <- amce(Credib2 ~ Wording + Label2 + Location + Severity + Type +
                     Wording*Label2,
                   data = medord, cluster = T, respondent.id = "ResponseId",
                   design = "uniform")
...
summary_table(medord[,covs], newsum) # paste into LaTeX 
