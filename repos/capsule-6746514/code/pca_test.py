"Classifier performs 10-fold cross validation on PCA"
...
if __name__=='__main__':
    ...
    for train_index, test_index in New_sam.split(New_data):
        ...
        opt = Adam(lr=0.001)
        model_0 = sgru_model(number_features=33, output_node=1, af=mish)
    ...
    for train_index, test_index in New_sam.split(New_data):
        ...
        opt = Adam(lr=0.001)
        model_1 = gru_model(number_features=33, output_node=1, af=mish)
        model_1.compile(loss='binary_crossentropy', optimizer=opt, metrics=['acc'])
        history_1 = model_1.fit(x_train, y_train, epochs=100, batch_size=32, validation_data=(x_test, y_test),
                                verbose=2)
