...
def fuzDist(a,b,cc):
    ...
    #fuz=abs(defuz(a,c)-defuz(b,c)) # D3 distance
    fuz=abs(defuz(fuzSubtr(a,b,cc),cc)) # D4 distance
    return fuz
...
def fuzMAE(a,b,cc):
    fuz=0
    for i in range(len(a)):
        fuz+=fuzDist(a[i],b[i],cc)
    fuz=fuz/len(a)
    return fuz

# Calculating of R-squared value according to given average of fuzzy numbers
def fuzR2(a,b,ave,cc):
    fuz1,fuz2=0,0
    for i in range(len(a)):
        fuz1+=fuzDist(a[i],b[i],cc)**2
        fuz2+=fuzDist(a[i],ave,cc)**2
    fuz=1-(fuz1/fuz2)
    return fuz
