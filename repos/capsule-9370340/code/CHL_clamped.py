...
# to plot error 
def plotLR(train_accs, test_accs, epoch):
  # to conver acc to errors
  train_errors = [(1 - i)*100 for i in train_accs]
  test_errors  = [(1 - i)*100 for i in test_accs ]
  
  plt.plot(train_errors, label='train set', linestyle='-',linewidth=4.0, color='b')
  plt.plot(test_errors, label='test set', linestyle='-',linewidth=4.0, color='g')
  ...
  plt.ylim([0, 100])
  xlabels = range(0, epoch)
  plt.xticks(xlabels)
  plt.savefig('../results/plotlc.png')
...
def preprocess_data():
  ...
  # these dataset are from "https://codeocean.com/capsule/8113806/tree/v2"
  x_train = np.load("/data/mnist/mnist_examples_train.npy")
  x_test  = np.load("/data/mnist/mnist_examples_test.npy")
  y_train = np.load("/data/mnist/mnist_targets_train.npy")
  y_test  = np.load("/data/mnist/mnist_targets_test.npy")
...
def run():
  ...
  epoch          = 3                      # training epoch you can change this for full training (601)
  ...
  # train a model
  train_accs, test_accs = train_model(epoch, w, b, learning_rate, gamma,  batch_size, minibatch_size, free_time, clamped_time, delay, dt, node_sizes, n_activations, prediction_inp_size, pred=pred)
  
  return train_accs, test_accs, epoch

# run the code
train_accs, test_accs, epoch = run()

# plot learning curve
plotLR(train_accs, test_accs, epoch)
