...
for train, test in kfold.split(Data, Label):
    ...
    # create model
    inputs = Input(shape=(SIZE2, SIZE1))
    inputs2 = Input(shape=(48,))
    net = Net()
    outputs, _ = net.hunhe(inputs, inputs2, keep_prob=keep_prob, num_classes=1)
    model = Model(inputs=[inputs, inputs2], outputs=outputs)
    opt = optimizers.SGD(lr=lr_schedule(0), momentum=momentum)
    model.compile(optimizer=opt, loss="binary_crossentropy",
                  metrics=['accuracy'])

    model.summary()

    metrics = Metrics(filepath=MODEL_PATH + model_name)

    lr_scheduler = LearningRateScheduler(lr_schedule)
    # 创建一个实例history
    # history = LossHistory()

    callback_lists = [metrics, lr_scheduler]

    class_weight = {0.1: 0.1, 0.9: 0.9}
    # class_weight = {0.05:0.1, 0.9500000000000001:0.9}
    model.fit(x=[train_f_pro, dwt_trainf], y=train_label, batch_size=batch_size, epochs=epochs, verbose=0,
              class_weight=class_weight,
              validation_data=([test_f_pro, dwt_testf], test_label),
    ...
    train_zzz = model.predict([train_f_pro, dwt_trainf], batch_size=batch_size, verbose=0)
    print(train_zzz.shape)
    test_zzz = model.predict([test_f_pro, dwt_testf], batch_size=batch_size, verbose=0)
    print(test_zzz.shape)
    ...
    train_label = smooth_labels(train_label, factor=0.2)

    del_files('../results/Other/')
    model_name2 = '2_net_lead_' + str(index) + '.hdf5'
    MODEL_PATH2 = '../results/Other/'

    inputs3 = Input(shape=(1,))
    inputs4 = Input(shape=(4,))
    net = Net()

    outputs, _ = net.hunhe32(inputs3, inputs4, keep_prob=keep_prob, num_classes=class_num)
    model2 = Model(inputs=[inputs3, inputs4], outputs=outputs)

    opt = optimizers.SGD(lr=lr_schedule2(0), momentum=momentum)
    ...
    # 创建一个实例history
    # history = LossHistory()
    metrics2 = Metrics(MODEL_PATH2 + model_name2)

    callback_lists = [metrics2, lr_scheduler]

    class_weight = {0.1: 0.1, 0.9: 0.9}
    history2 = model2.fit(x=[train_zzz, eng_trainf], y=train_label, batch_size=batch_size, epochs=epochs, verbose=0,
                          class_weight=class_weight,
                          validation_data=([test_zzz, eng_testf], test_label), callbacks=callback_lists)

    model2 = load_model(MODEL_PATH2 + model_name2,
                        custom_objects={'Capsule': Capsule, "AttentionWithContext": AttentionWithContext,
                                        "Self_Attention": Self_Attention})

    pred_vmm = model2.predict([test_zzz, eng_testf], batch_size=batch_size, verbose=0)
    pred_v = np.round(pred_vmm)
    true_v = test_label
